...
sleepRouter.get("/routine", requireAuth, (req, res) => {
  const db = getDb();
  let routine = db
...
sleepRouter.put("/routine", requireAuth, (req, res) => {
  ...
  if (!routine) {
    ...
    db.prepare("UPDATE sleep_routines SET title = ? WHERE id = ?").run(parsed.data.title, routine.id);
  }
  ...
  for (const id of existing) {
    if (!incomingIds.has(id)) {
      db.prepare("DELETE FROM sleep_routine_items WHERE id = ?").run(id);
    }
  }
  ...
  incoming.forEach((it, idx) => {
    ...
    const exists = it.id ? db.prepare("SELECT id FROM sleep_routine_items WHERE id = ?").get(it.id) : null;
    if (exists) {
      db.prepare("UPDATE sleep_routine_items SET label = ?, sort_order = ? WHERE id = ?").run(
        it.label,
        idx,
        id
      );
...
sleepRouter.put("/checkin", requireAuth, (req, res) => {
  const BodySchema = z.object({
  ...
  if (existing) {
    db.prepare("UPDATE sleep_item_checkins SET is_done = ? WHERE id = ?").run(isDone ? 1 : 0, existing.id);
...
sleepRouter.put("/settings", requireAuth, (req, res) => {
  const BodySchema = z.object({
  ...
  const existing = db.prepare("SELECT user_id FROM sleep_settings WHERE user_id = ?").get(req.user.id);
  if (existing) {
    db.prepare(
...
sleepRouter.get("/settings", requireAuth, (req, res) => {
  const db = getDb();
  const row = db
